...
export default function DiscoverPage({ lang, onSelectListing, onCreateListing, isAuthenticated, listings, loading, error, displayCurrency, onRetry }: Props) {
  ...
  return (
    <div className="flex flex-col h-full pixel-bg">
      {/* Header */}
      <div className="page-container px-4 md:px-6 lg:px-8 page-header pb-3 shrink-0 page-header-bar sticky top-0 z-10">
        <div className="flex items-center justify-between mb-3 gap-4">
          <div>
            <h1 className="font-display text-xl font-bold page-title">{t.discoverTitle}</h1>
          </div>
          {isAuthenticated && (
        ...
        <div className="relative mb-2">
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground opacity-60" width="14" height="14" viewBox="0 0 24 24" fill="none">
            <rect x="3" y="3" width="12" height="12" stroke="currentColor" strokeWidth="2" />
            <path d="M17 17L21 21" stroke="currentColor" strokeWidth="2" strokeLinecap="square" />
          </svg>
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            aria-label={t.searchPlaceholder}
            placeholder={t.searchPlaceholder}
            className="w-full pl-8 pr-4 py-2.5 text-sm field-input font-body" />
        </div>

        {/* Filter row */}
        <div className="flex gap-2 overflow-x-auto md:overflow-visible md:flex-wrap pb-1 scroll-hide">
          <button onClick={() => setShowFilters(!showFilters)}
            className={`flex items-center gap-1 px-3 py-1.5 text-xs font-semibold shrink-0 chip-btn ${showFilters || activeFilters > 0 ? 'chip-btn-active' : ''}`}>
            ⚙ {t.filter}
            {activeFilters > 0 && (
              <span className="ml-1 px-1.5 rounded text-[9px] font-pixel chip-count">{activeFilters}</span>
            )}
          </button>
          {(['newest', 'popular', 'price_asc', 'price_desc'] as const).map(s => (
            <button key={s} onClick={() => setSortBy(s)}
              className={`px-3 py-1.5 text-xs font-semibold shrink-0 chip-btn ${sortBy === s ? 'chip-btn-active' : ''}`}>
              {sortLabels[s]}
            </button>
          ))}
        </div>
        ...
        {showFilters && (
          <div className="mt-2 p-3 grid grid-cols-1 sm:grid-cols-3 gap-2 animate-slide-up filter-panel">
            {[
              ...
              <select key={i} value={sel.value} onChange={e => sel.onChange(e.target.value)} aria-label={sel.placeholder}
                className="text-xs field-select py-1.5 font-body">
      ...
      <div className="flex-1 overflow-y-auto scroll-end-buffer px-3 md:px-6 lg:px-8">
        ...
        {error && (
          <div className="text-center py-8">
            <p className="text-sm mb-3 text-danger">{t.networkError}: {error}</p>
        ...
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="font-display text-sm text-primary">{t.noResults}</p>
...
function ListingCard({ listing, t, lang, displayCurrency, onClick }: { listing: Listing; t: (typeof i18n)[AppLang]; lang: AppLang; displayCurrency: Currency; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="text-left overflow-hidden card-hover listing-card"
      ...
      {/* Info */}
      <div className="p-2">
        <p className="text-xs font-semibold text-foreground leading-tight line-clamp-2 mb-0.5 font-display">{listing.productName}</p>
...
function conditionColor(c: CardCondition) {
  return { M: 'var(--color-success)', NM: '#69F0AE', LP: 'var(--color-warning)', MP: '#FF9800', HP: 'var(--color-danger)' }[c] || 'var(--color-muted-foreground)';
}
